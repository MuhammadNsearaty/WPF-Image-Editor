    ...
    class Program {
        ...
        public static class Comunicator {
            ...
            public static IRestResponse<List<FeedItem>> GetFeedItems(User user) {
                ...
                var response = client.Get<List<FeedItem>>(request);
                return response;
            }
            ...
            public static FileStream DownLoadEnhancedImage(User user, FeedItem feedItem) {
                ...
                if (response.IsSuccessful) {
                    JsonObject json = JsonConvert.DeserializeObject<JsonObject>(client.Get(request).Content);
                    if ((bool)json["state"]) {
        ...
        public class User {
            ...
            public void Login() {
                ...
                if (response.IsSuccessful) {
                    ...
                    List<JsonObject> list = JsonConvert.DeserializeObject<List<JsonObject>>(client.Get(request1).Content);
        ...
        static void Main1(string[] args) {
            ...
            //var request = new RestRequest("/api/feed/", DataFormat.Json);
            //request.AddHeader("Authorization", "Token " + authToken.token);
